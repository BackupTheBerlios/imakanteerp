...
public class caseOpOut  extends dbObject {
    //-------------START MyVariables
    ...
    private int indexDoc[] = null; // vid doc razhodni  orderi
...
//
    ...
    public caseOpOut(java.sql.Connection conn, int level, int sdtn) {
        super(conn);
        this.setLevel(level);
    ...
    protected void prepareCstm() {
        try {
            ...
            setCstm(getConn().prepareCall("{call sl_procedure_case_out(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}"));
