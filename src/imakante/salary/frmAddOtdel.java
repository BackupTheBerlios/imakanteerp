...
public class frmAddOtdel extends javax.swing.JDialog {
    
    public frmAddOtdel(javax.swing.JFrame parent, boolean modal, java.sql.Connection con, int id_t) {
        super(parent, modal);
    ...
    protected void insertRecord(){
        if ( id == 0){
            try{
                ...
                stm.execute("INSERT INTO ls_otdeli (nomer, nameotdel) VALUES('"+  jTextField1.getText() + "', '" +jTextField2.getText()+ "')");
            }catch(java.sql.SQLException sqle){sqle.printStackTrace();}
        }else{
            try{
                stm = dbInternal.createStatement();
                stm.execute("UPDATE ls_otdeli SET nomer = '"+  jTextField1.getText() + "', nameotdel = '" +jTextField2.getText()+ "' WHERE id =" +id);
            }catch(java.sql.SQLException sqle){sqle.printStackTrace();}
        }
        imakante.salary.FrmOtdel.repaintTable();
        this.dispose();
    ...
    public void revalidateTextFields(){
        ...
        if (id!=0){
            try{
                ...
                stm = dbInternal.createStatement();
                rs = stm.executeQuery("SELECT * FROM ls_otdeli WHERE id = " + id);
                while(rs.next()){
                    jTextField1.setText(rs.getString("nomer"));
                    jTextField2.setText(rs.getString("nameotdel"));
                    
                }
            }catch(java.sql.SQLException sqle){sqle.printStackTrace();}
        }else{
            int nom = imakante.com.pubMethods.getMaxNum("SELECT nomer FROM ls_otdeli",dbInternal, "nomer") + 1;
