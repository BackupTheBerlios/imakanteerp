...
CREATE VIEW `mida`.`VAT_dr` AS
select `mida`.`sl_document_facade`.`number_df` AS `number_df`,
`mida`.`n_type_doc`.`name_ntd` AS `name_ntd`,
`mida`.`n_contragent`.`name_n_contragent` AS `name_n_contragent`,
`mida`.`n_contragent`.`bul_n_contragent` AS `bul_n_contragent`,
`mida`.`sl_document_facade`.`date_edition_df` AS `date_edition_df`,
`mida`.`sl_document_facade`.`total_df` AS `total_df`,
`mida`.`sl_document_facade`.`dds_df` AS `dds_df`
from ((`mida`.`sl_document_facade`
join `mida`.`n_contragent` on((`mida`.`n_contragent`.`id_contragent` = `mida`.`sl_document_facade`.`in_contragent_df`)))
join `mida`.`n_type_doc` on((`mida`.`n_type_doc`.`code_ntd` = `mida`.`sl_document_facade`.`type_df`)))
where (`mida`.`sl_document_facade`.`type_df` = 4)
order by `mida`.`sl_document_facade`.`number_df`
