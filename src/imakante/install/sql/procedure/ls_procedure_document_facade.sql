...
IF (comprator = 0) THEN
        SELECT
         ...
         LEFT JOIN n_obekt obkt_in ON s.in_obekt_df = obkt_in.id_n_obekt
         LEFT JOIN n_obekt obkt_out ON s.out_obekt_df = obkt_out.id_n_obekt
         LEFT JOIN ls_n_person p_deliv ON p_deliv.id_ls_n_person = s.delivere_df
         LEFT JOIN ls_n_person p_dist ON p_dist.id_ls_n_person = s.distributor_df
         LEFT JOIN n_contragent contr_in ON contr_in.id_contragent = s.in_contragent_df
         LEFT JOIN n_contragent contr_out ON contr_out.id_contragent = s.out_contragent_df
         LEFT JOIN ls_n_person p_contr_in ON contr_in.id_mol = p_contr_in.id_ls_n_person
         LEFT JOIN ls_n_person p_contr_out ON contr_out.id_mol = p_contr_out.id_ls_n_person
         LEFT JOIN n_doc_type_user_rights usr_new ON s.user_df = usr_new.id_ndtur
         LEFT JOIN n_doc_type_user_rights usr_last ON s.user_last_df = usr_last.id_ndtur
         LEFT JOIN sl_doc_type_num sldtn ON sldtn.id_sdtn = usr_new.id_sdtn
...
IF (comprator = 1) THEN
         INSERT INTO sl_document_facade(in_contragent_df, out_obekt_df, in_obekt_df, number_df, type_df, condition_df,
...
IF (comprator = 2) THEN
          UPDATE sl_document_facade s SET
...
IF (comprator = 3) THEN
      #   DELETE FROM sl_document_facade WHERE id_df = in_id_df;
      UPDATE sl_document_facade s SET s.condition_df ="1"
...
IF (comprator = 4) THEN
         ...
         FROM sl_document_facade s LEFT JOIN n_obekt obkt_in ON s.in_obekt_df = obkt_in.id_sno
         LEFT JOIN ls_n_person p_deliv ON p_deliv.id_ls_n_person = s.delivere_df
         LEFT JOIN ls_n_person p_dist ON p_dist.id_ls_n_person = s.distributor_df
         LEFT JOIN n_contragent contr_in ON contr_in.id_contragent = s.in_contragent_df
         LEFT JOIN ls_n_person p_contr_in ON contr_in.id_mol = p_contr_in.id_ls_n_person
...
IF (comprator = 6) THEN
        SELECT MAX(number_df) AS maxNumber FROM sl_document_facade s, sl_doc_type_num sl, n_doc_type_user_rights ur
...
IF (comprator = 7) THEN
          SELECT MAX(id_df) as id FROM sl_document_facade s;
END IF;
...
IF (comprator = 8) THEN
       IF (in_docFacadeType = 13) THEN
          ...
          l.comment_ls_n_person FROM ls_n_person l
...
IF (comprator = 20) THEN
   IF (in_docFacadeType = 0) THEN
     ...
     FROM n_product_main n LEFT JOIN n_product_consigment pc ON pc.id_pm = n.id_pm
     LEFT JOIN sl_nalichnosti s ON pc.id_pc = s.id_pc
     LEFT JOIN n_storage st ON st.id_n_storage = s.id_n_storage
...
IF (comprator = 22) THEN
   SELECT n.id_pd, n.m1_pd, n.v1_pd, n.m2_pd, n.v2_pd, n.m3_pd, n.v3_pd FROM n_product_description n
...
IF (comprator = 24) THEN
    ...
    FROM n_product_all_measure n WHERE id_pam = in_id_obekt_in;
END IF;

IF (comprator = 25) THEN
    SELECT * FROM sl_nalichnosti s LEFT JOIN n_product_consigment n ON n.id_pc=s.id_pc
    LEFT JOIN n_product_main pm ON pm.id_pm = n.id_pm
    WHERE n.id_pc =in_id_obekt_in AND pm.flag_pm = in_id_df;
END IF;

IF (comprator = 26) THEN
    INSERT INTO sl_document_lines(id_df,id_pc,id_n_storage,singly_price_dl,climb_down_dl,
...
IF (comprator = 29) THEN
    SELECT s.id_sl_curs, s.id_n_money, s.value_sl_curs,cod_lat_n_money FROM sl_curs s LEFT JOIN n_money m ON m.id_n_money = s.id_n_money
    WHERE s.id_sl_curs = in_id_obekt_in;
END IF;

IF (comprator = 30) THEN
    UPDATE sl_nalichnosti s SET s.quant_nal=(quant_nal-in_id_obekt_in)
      , s.quant_rezerv_nal=quant_rezerv_nal-in_id_obekt_in
    WHERE id_n_storage IN
                      (SELECT  dl.id_n_storage FROM sl_document_lines dl
                       WHERE dl.id_dl = in_id_df)
            AND id_pc IN
                     (SELECT  dl.id_pc FROM sl_document_lines dl
                       WHERE dl.id_dl = in_id_df);
...
n.id_pc, n.id_pm, n.parcel_pc, n.dateofexpire_pc,
 ...
 pam1.sname_pam as m1, pam2.sname_pam as m2, pam3.sname_pam as m3
    FROM sl_document_lines s LEFT JOIN n_product_consigment n ON s.id_pc = n.id_pc
    LEFT JOIN n_product_main pm ON pm.id_pm=n.id_pm
    LEFT JOIN  n_product_price pp   ON pm.id_pp=pp.id_pp
    LEFT JOIN  n_product_description pd   ON pm.id_pd=pd.id_pd
    LEFT JOIN  n_product_fee pf   ON pm.id_pf=pf.id_pf
    LEFT JOIN  n_storage st   ON st.id_n_storage = s.id_n_storage
    LEFT JOIN  n_product_all_measure pam1 ON pam1.id_pam = pd.m1_pd
    LEFT JOIN  n_product_all_measure pam2 ON pam2.id_pam = pd.m2_pd
    LEFT JOIN  n_product_all_measure pam3 ON pam3.id_pam = pd.m3_pd
    WHERE id_df = in_id_df;
END IF;

IF (comprator = 32) THEN
    UPDATE sl_document_lines s SET s.id_pc = in_id_obekt_in,
...
IF (comprator = 33) THEN
    DELETE FROM sl_document_lines
    WHERE  id_dl = in_id_df;
END IF;

IF (comprator = 34) THEN
    SELECT MAX(id_dl) as id FROM sl_document_lines s;

END IF;

IF (comprator = 35) THEN
  # DELETE FROM sl_document_facade
...
IF (comprator = 40) THEN
    UPDATE sl_document_facade s SET s.condition_df ="2"
    WHERE s.id_df = in_id_df;
END IF;
