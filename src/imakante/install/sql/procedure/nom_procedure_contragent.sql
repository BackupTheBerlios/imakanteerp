...
CREATE PROCEDURE `mida`.`nom_procedure_contragent`(IN comprator TINYINT,  IN in_id INT(10),      IN in_cod VARCHAR(11),     IN in_name VARCHAR(45),
...
BEGIN
     ...
     IF (comprator = 5) THEN
       SELECT c.id_contragent, c.code_contragent, c.name_n_contragent, c.bul_n_contragent, c.dan_n_contragent, c.address_n_contragent, c.id_n_nm, nm.name_n_nm, obl.name_n_oblast, cont.code_n_country, cont.name_n_country, c.tel_contragent, c.fax_contragent, c.email_contragent, c.web_contragent, c.id_mol,
                @MOL:=(select name_ls_n_person from `ls_n_person` where id_ls_n_person = id_mol) AS MOL,
               c.id_oso, @OSO:=(select name_ls_n_person from `ls_n_person` where id_ls_n_person = id_oso) AS OSO, c.flag_n_contragent
        FROM `n_contragent` c, `n_nm` nm, `n_oblast` obl , `n_country`  cont
        WHERE (c.id_n_nm = nm.id_n_nm and nm.id_n_oblast = obl.id_n_oblast and obl.id_n_country = cont.id_n_country and c.flag_n_contragent = in_flag ) AND
              ...
              c.address_n_contragent LIKE CONCAT('%',in_address,'%') AND c.tel_n_contragent LIKE CONCAT('%',in_tel,'%') AND c.fax_n_contragent LIKE CONCAT('%',in_fax,'%') AND
              c.email_n_contragent LIKE CONCAT('%',in_email,'%') AND c.web_n_contragent LIKE CONCAT('%',in_web,'%') ORDER BY c.code_contragent;
     END IF;
