...
DROP PROCEDURE IF EXISTS `mida`.`nom_procedure_incom` $$
...
BEGIN
     ...
     END IF;
     IF (comprator = 1) THEN
        INSERT INTO `mida`.`n_incoms`( id_n_group, code_n_incoms, name_n_incoms, comments_n_incoms) VALUES(in_id_groupe, in_code, in_name, in_comments);
     END IF;
     IF (comprator = 2) THEN
        UPDATE `mida`.`n_incoms` SET id_n_group = in_id_groupe, code_n_incoms = in_code,   name_n_incoms = in_name, comments_n_incoms = in_comments
        WHERE `mida`.`n_incoms`.id_n_incoms = in_id;
     END IF;

     IF (comprator = 3) THEN
        DELETE FROM `mida`.`n_incoms`  WHERE id_n_incoms = in_id;
     END IF;
     ...
     IF (comprator = 7) THEN
        SELECT MAX(n.id_n_incoms) FROM `mida`.`n_incoms` n;
     END IF;

     IF (comprator = 8) THEN
        SELECT MAX(n.code_n_incoms) AS code_n_incoms FROM `mida`.`n_incoms` n;
     END IF;
